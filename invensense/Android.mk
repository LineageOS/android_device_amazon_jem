...
LOCAL_MODULE := libinvensense_hal_bowser
...
LOCAL_SHARED_LIBRARIES += libmllite_bowser

#Additions for SysPed
LOCAL_SHARED_LIBRARIES += libmplmpu_bowser
...
include $(CLEAR_VARS)
LOCAL_MODULE := libmplmpu_bowser
LOCAL_SRC_FILES := libmplmpu_bowser.so
LOCAL_MODULE_TAGS := optional
...
include $(CLEAR_VARS)
LOCAL_MODULE := libmllite_bowser
LOCAL_SRC_FILES := libmllite_bowser.so
LOCAL_MODULE_TAGS := optional
LOCAL_MODULE_SUFFIX := .so
LOCAL_MODULE_CLASS := SHARED_LIBRARIES
LOCAL_MODULE_PATH := $(TARGET_OUT)/lib
